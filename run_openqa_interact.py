...
def main():
    ...
    _model_class = ReClassifyGlobalQA
    _trainer_class = RerankerTrainer
    _train_class = TrainDatasetOpenQAInteract
    _eval_class = PredictionDatasetOpenQAInteract
    _test_class = PredictionDatasetOpenQAInteract
    ...
    def formate_output(pred_scores, outfile):
        ...
        with open(data_args.pred_id_file, 'r', encoding='utf-8') as f:
            for l in f:
                ins = json.loads(l)
                labeled_data.append(ins)
        if trainer.is_world_process_zero():
            assert len(labeled_data) == len(pred_scores)
            all_scores = defaultdict(dict)
            for obj, score in zip(labeled_data, pred_scores):
                qid = obj['qry']
                for did, lb, ss in zip(obj['psgs'], obj['labels'], score):
                    all_scores[qid][did] = (ss, lb)
            qq = list(all_scores.keys())
            with open(outfile, "w") as writer:
                for qid in qq:
                    score_list = sorted(list(all_scores[qid].items()), key=lambda x: x[1][0], reverse=True)
                    for rank, (did, score) in enumerate(score_list):
                        writer.write(f'{qid}\t{did}\t{rank + 1}\t{score[0]}\t{score[1]}\n')
    ...
    if training_args.do_train and trainer.is_world_process_zero():
        ...
        for case in train_dataset[random.choice(range(train_dataset.__len__()))][:2]:
            ...
            print(case)
        for case in eval_dataset[random.choice(range(eval_dataset.__len__()))][:2]:
            input_ids = case['input_ids']
            print(' '.join(tokenizer.convert_ids_to_tokens(input_ids)))
            print(case)
    # Training
