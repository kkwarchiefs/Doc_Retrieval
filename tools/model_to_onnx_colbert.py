...
class RetrieverInfer(nn.Module):
    ...
    def from_pretrained(cls, *args, **kwargs):
        ...
        model = cls(hf_model)
        prefix_state_dict = torch.load(os.path.join(RM_model_path, "pytorch_model.bin"))
        new_prefix_state_dict = {}
        for k, v in prefix_state_dict.items():
            if k.startswith("linear."):
                new_prefix_state_dict[k[len("linear."):]] = v
        model.linear.load_state_dict(new_prefix_state_dict)
        return model
